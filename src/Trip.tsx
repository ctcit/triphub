...
export class Trip extends Component<{
    ...
    public getParticipantsInfo(): IParticipantsInfo {
        ...
        const early = moveable.slice(0, maxParticipants - leaders.length)
        const late = moveable.slice(maxParticipants - leaders.length)
    ...
    public getMapSummary(): string {
        return this.getRouteSummary() + ', ' +
            (this.getMaps().length === 0
                ? 'no maps selected'
                : 'selected maps: ' + this.getMaps().map(m => m.split(' ')[0]).join(', '))
    }
    ...
    public render() {
        ...
        const email = () => <Email key={`Email${trip.id}`} owner={this} app={this.props.app} />
        ...
        const navbar = <TriphubNavbar key='triphubnavbar' app={this.props.app}>
        ...
        return [
            ...
            this.props.isNew || !this.isPrivileged() ? null :
                <Expandable key={`email${this.state.trip.id}_${this.state.participants.length}`} id='email'
                    ...
                    level={2} expanded={false} onDemand={email}>
                    Email ...
                </Expandable>,
